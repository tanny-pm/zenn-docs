...
そんな経験はありませんか？（私はよくあります。朝は晴れている時によくやります 😇）というわけで今回は、雨の予報を絶対に見逃さないように、午後の降水確率に応じて廊下のライトの色を変える機能を作ってみました。さすがにライトの色が違えば気づくはず！

スマートライトは SwichBot のライトを利用します。API 経由で色を制御可能で、それでいて他社製のスマートライトより値段が安いので圧倒的におすすめです！
https://amzn.to/3Iod0MO

完成イメージは以下のような感じです。降水確率が高い時はライトが青く光ります。これを毎朝７時に定期実行します
...
2. [天気予報 API（livedoor 天気互換）](https://weather.tsukumijima.net)を利用して降水確率を取得する
...
SwitchBot API は公式のドキュメントも充実しているので、思ったより簡単にスマートライトを制御することができました！スマートライトを何かの通知に使うのは結構便利なので、他にも通知したい内容が思いついたら実装してみようと思います。

また今回はスクリプトの定期実行の方法として Github Actions を選択しました。ソースコードを Gihtub 上で管理している場合、YML ファイルを追加するだけで定期実行できるようになるので、非常にお手軽でした。
