...
Homebrewは、すべてのファイルを**プレフィックス**と呼ばれるディレクトリ以下に配置します。Apple SiliconのMacでは、`/opt/homebrew`がプレフィックスです（この記事もこの環境を前提に進めます）。
...
公式インストーラは、ファイルの配置まではやりますが、PATHを通すところは自分で確認する必要があります。一方でアップデートは、インストールしたツール自身がバックグラウンドで自動的に最新へ更新します。
...
- 意図しないタイミングで更新されないよう、バージョンを自分で握りたい
...
たとえばClaude Codeなら、放っておいても最新に保ちたいならバックグラウンドで自動更新される公式インストーラ、更新のタイミングを自分で握りたいなら`brew upgrade`で上げるHomebrew、という選び方になります。どちらも公式に用意された方法なので、運用スタイルに合わせて選べます。
